...
export async function DELETE(
    ...
    try {
        ...
        // Only allow hiding expired, missed, or cancelled reservations
        if (reservation.status !== 'EXPIRED' && reservation.status !== 'MISSED' && reservation.status !== 'CANCELLED') {
            return NextResponse.json(
                { error: 'Chỉ có thể ẩn giữ chỗ đã hết hạn, bỏ lỡ hoặc đã hủy' },
                { status: 400 }
            )
        }
        ...
        await prisma.reservation.update({
            where: { id: reservationId },
            data: {
                // Keep original status (EXPIRED, MISSED, or CANCELLED), just add hidden marker in notes
